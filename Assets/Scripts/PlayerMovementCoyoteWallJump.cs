...
public class PlayerMovementCoyotewallJump : MonoBehaviour{
    [SerializeField] private SpriteRenderer _spriteRenderer;
    [SerializeField] private AudioSource _jumpAudio;
    [Header("Movement")] [SerializeField] private float _xSpeed;
    [SerializeField] private float _jumpSpeed;
    [Header("Tweaks")][SerializeField] private float _upGravityScale = 1;
    ...
    private void Update(){
        ...
        if (Input.GetButtonDown("Jump")){
            //se non e' a terra
            if (!_isGrounded){
                //ho forse il walljump?
                if (_hasWallJump != 0){
                    velocityVector.y = Jump();
                    velocityVector.x = _hasWallJump * _xSpeed;
                }
                else{
                    //se non e' a terra, non ha walljump, ma e' nel coyote timer
    ...
    private void CheckWallJump(){
        //Se sono a terra non posso avere il walljump
        if (_isGrounded){
            _hasWallJump = 0;
            return;
        }

        //controllo se ho muri a destra o a sinistra
        RaycastHit2D hit1 = Physics2D.Raycast(transform.position + Vector3.up * 0.375f, Vector2.left, 0.26f, _groundLayer);
        if (hit1){
            _hasWallJump = 1;
            return;
        }

        RaycastHit2D hit2 = Physics2D.Raycast(transform.position + Vector3.up * 0.375f, Vector2.right, 0.26f, _groundLayer);
        if (hit2){
            _hasWallJump = -1;
            return;
        }
